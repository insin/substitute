 ...
 * @param pollInterval if given, starts loading tweets immediately and schedules
 *     regular loading at the given interval (in ms).
 */
function TweetScanner(user, auth, pollInterval) {
  // Config
  this.user = user
  this.auth = auth
  this.pollInterval = pollInterval || 60000
...
/**
 * Loads the first batch of tweets and schedules regular loading of tweets.
 */
TweetScanner.prototype.start = function() {
  this.loadTweets()
  this.intervalId = setInterval(this.loadTweets.bind(this), this.pollInterval)
  this.running = true
}
...
/**
 * Updates the poll interval and reschedules if we're already running.
 */
TweetScanner.prototype.setPollInterval = function(pollInterval) {
...
TweetScanner.prototype.processTweets = function(err, tweets) {
  ...
  this.info('Processing %s user timeline tweet%s...', tweets.length,
            pluralise(tweets.length))

  var scanner = this
  async.forEachSeries(tweets.reverse(), this.processTweet.bind(this), function(err) {
    if (err) {
      scanner.error('Automatically stopping - error processing tweets: %s', err)
      scanner.stop()
    }
    scanner.tweetsScanned += tweets.length
  })
}
